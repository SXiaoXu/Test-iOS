...
@property (nonatomic, strong) UIImageMessageViewController *imageVc;
...
- (void)viewDidLoad {
    ...
    self.listArray = [@[@"单聊",@"群聊",@"富媒体",@"存储",@"webView",@"用户反馈",@"餐厅系统",@"社交",@"文件上传",@"网络请求",@"测试短信",@"第三方登录分享",@"测试",@"发消息",@"收消息",@"应用内搜索"] mutableCopy];
    self.controllerArray =[@[self.singleVc,self.groupVC,self.imageVc,self.storeVc,self.webVc,self.restVc,self.restVc,self.followVc,self.fileVc,self.httpVc,self.messageVc,self.ThirdPattyVc,self.testVc,self.sendMsgVc,self.ReceiveMsgVc,self.searchVc] mutableCopy];
}
...
-(UIImageMessageViewController *)imageVc{
    if (!_imageVc) {
        _imageVc = [[UIImageMessageViewController alloc]initWithNibName:@"UIImageMessageViewController" bundle:nil];
...
-(FileViewController *)fileVc{
    if (!_fileVc) {
        _fileVc = [[FileViewController alloc]init];
    }
    return _fileVc;
}
...
-(SearchViewController *)searchVc{
    if (!_searchVc) {
        _searchVc = [[SearchViewController alloc]initWithNibName:@"SearchViewController" bundle:nil];
